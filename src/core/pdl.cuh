...
// Ampere fork (NINFER_DISABLE_PDL): PDL needs sm_90+. The producer/consumer pair always lives in
// the same stream, so a plain stream-ordered launch plus no-op device hooks is semantically
// identical (strictly less overlap, same ordering). All PDL uses are producer/consumer pairs in
// one stream; no cross-stream dependency relies on the trigger.
...
#ifdef NINFER_DISABLE_PDL
    // No-op: the stream-ordered consumer observes full producer completion.
#else
